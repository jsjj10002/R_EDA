...
group_by()와 summarise(), n()을 사용해 count()와와 동일한 결과를 낼 수 있다.
...
위와같이 group_by()와 필터링을 통해서도 찾을 수 있다. \### 3.7.4 각 목적지별 총 지연시간을 분으로 계산, 각 항공편별로 목적지까지의 총 지연시간의 비율 계산 \#### 목적지별 총 지연 시간
